...
const ATTACK_DISTANCE = 20.0   # Jarak sentuh untuk menyerang (piksel)
...
func attack():
	...
	# Apply damage immediately so looping attack animations
	# do not block damage delivery.
	if current_target != self and current_target.is_in_group("player") and current_target.has_method("take_damage") and current_target.has_method("get_hurtbox_global_rect") and _get_hitbox_global_rect().intersects(current_target.get_hurtbox_global_rect()):
